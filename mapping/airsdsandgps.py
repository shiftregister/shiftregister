...
# with # http://ecksteinimg.de/Datasheet/SDS011%20laser%20PM2.5%20sensor%20specification-V1.3.pdf sensor

# Adapted by Martin Howse...
...
GPSADDR = "/dev/ttyUSB0" # plug in first

# for sensor

ser = serial.Serial()
ser.port = "/dev/ttyUSB1" # plug in sensor second
ser.baudrate = 9600
...
#            self.port.open() # we needed to get rid of this?
        ...
        if record['sentence'] == 'RMC':
            ...
            if lastbyte == "\xAA" and byte == "\xC0":
                ...
                if record['longitude']!=0.0:
                    print record['longitude'],",", record['latitude'],",",pm_25,",",pm_10
